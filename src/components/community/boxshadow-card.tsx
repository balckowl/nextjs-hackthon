import type { SelectBoxShadow, SelectUser } from '@/db/schema'
...
type Props = {
    boxshadowWithUser: { "box-shadows": SelectBoxShadow, user: SelectUser | null }
    userId: string | null;
}

export default function Boxshadowcard({ boxshadowWithUser, userId: nowLoggedUserId }: Props) {
    const router = useRouter()

    const [isCopySuccess, setIsCopySuccess] = useState<boolean>(false)
    const { offsetX, offsetY, blurRadius, spreadRadius, shadowColor, createdAt, id, color, title, userId } = boxshadowWithUser["box-shadows"]
    const { name, image } = boxshadowWithUser["user"]!
        ...
        <Link href={`/box-shadow/${id}`} className="col-span-1">
            ...
            <div className="bg-white border-[2px] h-[220px] rounded-xl relative flex items-center justify-center mb-[10px] overflow-hidden">
                ...
                <div className='absolute top-3 right-3 flex gap-2'>
                    ...
                    {userId === nowLoggedUserId && <div onClick={handleDeleteBoxShadow} className='bg-white border-[2px] p-2 rounded-md'>
                        <HiOutlineTrash />
                    </div>}
                </div>
            </div>
